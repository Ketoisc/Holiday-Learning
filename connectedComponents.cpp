...
void connected_components(graph* graph) { // find all connected components using breadth first search on every undiscovered node in the graph (bfs will discover nodes that are connected through edges)
    int componentNum = 0;
    initialise_search(graph);
    for (int i = 0; i < graph->numVertices; i++) {
        if (graph->discovered[i] == false) { // if current node is undiscovered, run bfs to find all other connected nodes
            componentNum++;
