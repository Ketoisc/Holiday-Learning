...
void process_vertex_late(graph* graph, int vertex) { // pushes processed node onto stack
...
void process_edge(graph* graph, int x, int y) { // processes edges 
    ...
    if (edgeClass == "BACK") { // if it is a back edge, it is a cycle and not allowed for top sort
        cout << "Warning: directed cycle found (back edge), not a DAG" << endl;
        graph->finished = true;
    }
    return;
}
...
void topsort(graph* graph) {
    ...
    for (int i = 1; i <= graph->numVertices; i++) {
        if (graph->discovered[i] == false) {
            dfs(graph, i); // runs the dfs for all components
        }
    }
 // prints and reports the topological order
    while (!graph->dfsStack.empty()) {
        cout << graph->dfsStack.top() << " ";
        graph->dfsStack.pop();
    }
    return;
}
