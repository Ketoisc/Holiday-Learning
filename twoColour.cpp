...
struct graph {
    ...
    // arrays for traversal and search
    bool processed[6]; 
    bool discovered[6];
    int parent[6];
...
void process_edge(graph* graph, int x, int y) { // processes edges, checks for appropriate vertex colouring
    if (graph->colour[x] == graph->colour[y]) {
        graph->bipartite = false;
        cout << "Warning: not bipartite due to " << x << " " << y << endl;
        return;
    }
    ...
    while (!queue.empty()) { // continue searching until queue is empty/no more vertices left to search
        ...
        while (currNode != NULL) { // while there are still neighbours to the node/s, traverse all neighbours of currVert using its adjacency list
            ...
            // focuses on processing edges
                process_edge(graph, currVert, successorVert); // process the edge
            }
