...
void dijkstra(graph* graph, int start) {
    ...
    while (inTree[currVert] == false) { // continue making mst until all vertices are in tree
        ...
        while (tempNode != NULL) { // while there are still edges on this vertex, traverse the adjacency list of currVert
            ...
            // If the edge weight + distance of current vert is smaller than the current recorded distance (initially infinity) AND next one is not in the tree
            if (distance[potentialNextVert] > (distance[currVert] + weight)) { 
                distance[potentialNextVert] = distance[currVert] + weight; // set the next vertex's weight
